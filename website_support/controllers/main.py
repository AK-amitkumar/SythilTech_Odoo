...
class SupportTicketController(http.Controller):
	...
	for field_name, field_value in kwargs.items():
        ...
        if http.request.env.user.name != "Public user":
            ...
            new_ticket_id = request.env['website.support.ticket'].sudo().create({'person_name':values['person_name'],'category':values['category'], 'sub_category_id': sub_category, 'email':values['email'], 'description':values['description'], 'subject':values['subject'], 'can_continue_with_other_tasks':values['can_continue_with_other_tasks'], 'partner_id':http.request.env.user.partner_id.id, 'attachment': my_attachment, 'attachment_filename': file_name, 'portal_access_key': portal_access_key})
